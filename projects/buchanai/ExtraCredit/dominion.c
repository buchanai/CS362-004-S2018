...
int adventurerCard(struct gameState *state, int *temphand, int currentPlayer){ 
      int cardDrawn;
      int drawntreasure = 0;
      int z = 0;      

      while(drawntreasure<2){	//bug from assignment two repaired - 3 changed back to 2.
	if (state->deckCount[currentPlayer] <1){//if the deck is empty we need to shuffle discard and add to deck
	  shuffle(currentPlayer, state);
	}
	drawCard(currentPlayer, state);
	cardDrawn = state->hand[currentPlayer][state->handCount[currentPlayer]-1];//top card of hand is most recently drawn card.
	if (cardDrawn == copper || cardDrawn == silver || cardDrawn == gold || cardDrawn == village) //a more settle bug introduced here - will occur less freqently.
	  drawntreasure++;
	else{
...
int smithyCard(struct gameState *state, int currentPlayer, int handPos){
      //+3 Cards
      int i;
      for (i = 0; i < 5; i++)	//TODO: CHANGED 3 TO 5 - BUG INTRODUCTED HERE
	{
	  drawCard(currentPlayer, state);
	}
...
int villageCard(struct gameState *state, int currentPlayer, int handPos){
      ...
      //+2 Actions
      state->numActions = state->numActions + 3;	//TODO: CHANGED THE NUMBER OF ACTIONS FROM 2 TO 3
...
int greatHallCard(struct gameState *state, int currentPlayer, int handPos){
      //+1 Card
      drawCard(currentPlayer, state);
      drawCard(currentPlayer, state);		//TODO: BUG INTRODUCED HERE - SECOND drawcard function induced inappropriate gameplay functionality.
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  //These three variables were moved to the refactored adventure card code.
  //-------------------------
  //int drawntreasure=0;	
  //int cardDrawn;
  ...
  switch( card ) 
    ...
    //REFACTORED
    case adventurer:
      adventurerCard(state, temphand, currentPlayer);
      return 0;
