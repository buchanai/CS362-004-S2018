...
int testCard(struct gameState* st, struct gameState *pre, int currentPlayer){
	...
	//Adding variables to accomodate Ashley's function arguments.
	int drawntreasure = 0;
	int cardDrawn = 0;
	int x = 0; //counter for temp hand

	//call the adventurer card.
	adventurerCard(drawntreasure, st, currentPlayer, cardDrawn, temphand, x);

	//Remove return value test.
